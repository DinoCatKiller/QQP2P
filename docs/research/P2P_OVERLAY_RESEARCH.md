...
### 3.3 虚拟网卡（档次B）的一键安装调研（2026-08）

> 背景：档次B 唯一劝退点是"驱动/权限/安装复杂"。调研结论：**一键化是成熟套路**，2026 年所有环节均可自动化，用户负担可压到"双击 exe → 点一次『是』→ 完事"。唯一物理上绕不开的：Windows 创建虚拟网卡 = 内核操作 = **必须一次 UAC（管理员权限弹窗）**。

#### 3.3.1 三条技术路线（按省事程度排序）

| 路线 | 做法 | 用户负担 | 自研工作量 | 适用 |
|------|------|---------|-----------|------|
| **路线1：现成产品 MSI + 引导器（推荐）** | 自己程序只当"引导器"：检测 → 调官方 MSI 静默装 → CLI 一行加网 → 上报虚拟 IP | **一次 UAC** | 小（只写引导器 + 失败回滚） | 大多数场景 |
| **路线2：wintun + 自实现 WireGuard** | wintun 驱动免安装（Win10 1803+ 随 dll 动态加载，微软已签名），但协议栈/网卡/路由全自写 | 一次 UAC（建接口） | 大（相当于重写一遍 Tailscale） | 洁癖 / 坚决零第三方软件 |
| **路线3：libzt 嵌入** | ZeroTier 编译进 exe，界面看不到"客户端"，但驱动安装仍逃不掉 | 一次 UAC（你的 exe 偷偷装驱动） | 中 | 想"隐藏产品存在" |

**路线1 具体命令配方（官方参数已确认稳定）：**

```
# Tailscale：静默安装 → 一行加入网络 → 取虚拟 IP
msiexec /i tailscale-setup-x.y.z-amd64.msi /qn
tailscale up --authkey=tskey-auth-xxxx --hostname=bot-A
tailscale ip -4                      # → 100.x.x.x

# ZeroTier：同样配方
zerotier-one_x.x.x_x64.msi /qn
zerotier-cli join <network_id>
```

> 驱动、虚拟网卡、路由、防火墙全部由官方安装包自动处理——这类产品本来就是面向小白的一键设计，我们只是把"装 + 加网 + 上报"串成一条无人值守流水线。

#### 3.3.2 用户视角的一键体验

```
用户双击 bot.exe（或 setup.exe）
→ 弹出"需要安装虚拟网卡驱动，是否继续？"（UAC，点一次『是』）
→ 进度条 3 秒（静默装 Tailscale/ZeroTier + 自动加入你的网络）
→ 完成通知："组网成功，你的虚拟 IP 是 100.64.x.x"
→ 之后无感，bot 用 QQ 信令 + 虚拟 IP 通信
```

#### 3.3.3 两个绕不开的隐藏成本

1. **一次 UAC 逃不掉**：Windows 虚拟网卡 = 内核操作 = 管理员权限，操作系统规则，不是产品能绕过的（路线2 的 wintun 建接口也要管理员）。但只要**只有一次**，对用户就不是负担——装微信也要点 UAC。
2. **数字签名**：未签名 exe 触发 SmartScreen"未知发布者"警告。个人开发者选项：买 OV 代码签名证书（几百元），或自签名 + 引导用户"更多信息→仍要运行"。这是任何分发包都绕不开的成本。

#### 3.3.4 决策提醒：什么时候才值得虚拟网卡

```
想要「任意软件都能互访」（远程桌面/SMB/游戏/数据库）→ 虚拟网卡值得，路线1 够简单
想要「两台机器人之间的稳定通道」              → iroh 就是"进程内虚拟网卡"，零安装零 UAC
```

**如果只是机器人之间通信，iroh 连"一键"都省了**：`cargo add` 完事，用户根本不知道有这层存在。虚拟网卡只在"不是你的程序也要互访"时才值回那一次 UAC + 驱动 + 签名维护的成本。
